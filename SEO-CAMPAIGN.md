...
- Before every SEO campaign action run `pnpm seo:goal:control`. Stop on failure.
  The program checks the complete contracts and ledger without printing them.
...
- Existing-owner SEO rewrites require confirmed post-deploy crawl, 20 page
  impressions and 3 qualified joined-query impressions in the same complete
  28-day GSC range, plus the 28-complete-day post-crawl cooldown. Read the content
  bookmarks for exceptions and net-new eligibility before selecting work.
- Local research or repair does not authorize push, merge, deployment, indexing,
  external publication, paid acquisition or analytics mutation. Honor the exact
  user-approved scope; old approvals and dates do not grant new authority.
- Use the native Goal lifecycle. Finish useful authorized work, then follow the
  native blocked audit and retain the existing wakeup; do not poll the timer.

### Read only when the task requires it

Open each matching bookmark before acting; several rows may apply. Read the
named section through the next heading of equal or higher level, not its whole
file. Record the source section and dated evidence in the action's existing
record. If a link, evidence or current state is missing/conflicting, resolve it
before dependent work. A program PASS does not certify that a model read or
correctly applied the linked instructions.

| Next action | Required bookmarks |
|---|---|
| Resume / choose next work | [Successor deadline and stop conditions](SEO-CAMPAIGN.md#successor-goal-contract); current Space Brief; [recovery queue](docs/seo-growth-recovery.md); exact relevant record in [experiment ledger](EXPERIMENTS.md). Old "current" headings are not fresh state. |
| Choose content or distribution | [Six demand questions](docs/seo-product-evidence-standard.md#demand-decision-before-implementation), [candidate gate](SEO-CAMPAIGN.md#candidate-gate), [acquisition focus](SEO-CAMPAIGN.md#acquisition-focus). |
...
| Create a new or translated page | [Content correction](SEO-CAMPAIGN.md#protected-trilingual-scenario-expansion-correction); check canonical overlap in [scenario source](docs/seo-scenario-backlog.json). |
| Implement / verify a page | [Page delivery](docs/seo-product-evidence-standard.md#page-delivery), [product evidence](docs/seo-product-evidence-standard.md#product-evidence-rules), [visual acceptance](docs/seo-product-evidence-standard.md#responsive-and-visual-acceptance), [checks](docs/seo-product-evidence-standard.md#verification-gates). |
| Measure / declare an outcome | [Query milestones](docs/seo-product-evidence-standard.md#added-value-and-query-level-ranking-goals), [Successor contract](SEO-CAMPAIGN.md#successor-goal-contract), [quality conditions](SEO-CAMPAIGN.md#quality-conditions), [evidence roles](SEO-CAMPAIGN.md#evidence-roles-and-demand-discovery); exact experiment and latest completed weekly evidence, including each locale and crawl window. |
...
`SEO-CAMPAIGN.md` preserves contracts and existing history. Do not append session logs,
raw output, copied audits or unrelated discussion to it. Keep current work in the
recovery queue, experiment results in the append-only ledger, and dated evidence
in audits; private session notes belong in Wenlan. Link rather than duplicate.
`pnpm seo:goal:control --update` regenerates the guide only; it grants no action
permission. The guide has a 6,000-byte ceiling; move detail to a bookmark rather
than raising the limit. Full-contract checks continue even when only this guide
is printed. Do not add another scheduler, plan layer or recurring history read.
