...
The research task passes demand discovery and is the highest-leverage audience-oriented candidate currently found. It is now recorded as the eighth protected scenario family and must be prepared as one trilingual task with separate natural language—not as three literal translations.
...
Each locale owns the same bounded research task in natural local search language, links from the document-ingestion, citation-verification, and source-backed knowledge-base owners, and keeps the prohibited claims above visible. The sitemap intent map now has one owner for each of `129/129` URLs.

Local verification passed `pnpm seo:goal:check`, `pnpm seo:scenario:check`, `pnpm test:goal`, `pnpm test:i18n`, `pnpm lint`, `pnpm build`, and `pnpm seo:technical:built`. The focused RED-first scenario, intent, and i18n contract suite passed `73/73` after implementation. Desktop `1280x720` and mobile `393x852` production renders were checked for all three locales with correct canonical, locale routing, Article/BreadcrumbList schema, visible FAQ without FAQPage schema, and no document-level horizontal overflow or CJK glyph failure.

The full `pnpm test:seo` suite remains blocked by three pre-existing release-alignment assertions: the current sibling Wenlan source reports `0.17.2` while this dirty website worktree still contains `0.15.8` release metadata. The new scenario tests pass; this receipt does not silently repair that unrelated release state.

Website publication, commit, push, PR, merge, Vercel deployment, request indexing, GSC validation, and the external directory proposal remain separately approval-gated.
