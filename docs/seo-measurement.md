...
- The homepage build serves the README-aligned copy with `LME_Oracle` `93.6%` Recall@5 / `0.857` MRR / `0.883` NDCG@10, `LME_S` `87.7%` Recall@5 / `0.815` MRR / `0.822` NDCG@10 on the stratified `N=90` deep-S substrate, `softwareVersion` `0.12.0`, and the current `wenlan mcp add` setup wording.
